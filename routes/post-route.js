...
router.get("/write", checkAuthentication, (req, res) => {
  res.render("write-blg", { user: req.user });
});
...
var slug =req.params.slug;
  ...
  var deletedPost = await Post.deleteMany({
    _id: post._id,
    author: req.user._id,
    slug
  });
...
var patt = /\\|\/|\<|\>|\?|\"|\'/ig;
if(patt.test(req.body.title)){
  req.flash("Error","There should not be any special characters in title");
  return res.redirect("/post/write");
}
console.log(patt.test(req.body.title));
